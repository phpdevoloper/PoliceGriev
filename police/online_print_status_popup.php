...
$(document).ready(function(){
	...
	$.ajax({
		type: "POST",
		url: "online_action_status_list.php",
		...
		var  action_doc_tr = show_action_doc_show+
			...
			"<td style='background-color: #dddddd;'>"+"<span id='span_dwnd'  onClick='action_doc("+$(xml).find('action_doc_id').eq(0).text()+")'>"+$(xml).find('action_doc_name').eq(0).text()+"</sapn></td>"+
			"</tr>";
			var fir_csr_det = 
			...
			"</tr>";
			}var  link_petition_status_tr ="";
			if ($(xml).find('link_petition_status').eq(0).text() != '') {
				var  link_petition_status_tr = "<tr><td style='background-color: #dddddd;'>"+"<?php echo $lang['LINK_PET_STATUS']; ?>"+"</td>"+
...
"<tr>"+
	"<td>"+comm_label+"</td>"+
	"<td>"+comm_detail+"</td>"+
"</tr>"+doc_tr+action_doc_tr+fir_csr_det+link_petition_status_tr+
"<tr>"+
	"<td style='color: #ffffff;font-weight: bold;background-color: #8d7e7e;'>"+"<?php echo $lang['STATUS']; ?>"+"</td>"+
	"<td style='color: #ffffff;font-weight: bold;background-color: #8d7e7e;'>"+$(xml).find('Status').eq(0).text()+"</td>"+
"</tr>"+ 
/* "<tr>"+
...
<body onload="noBack();" >
<?php
$petno=stripQuotes(killChars(trim($_POST['petition_id1'])));
