...
<?php
	...
	if(!empty($grie_dept_id)) {		
  	...
  	$src_condition = "";
	if(!empty($src_id)) {
		$src_condition = " (b.source_id=".$src_id.")";
	}
	if (!empty($src_id)&& !empty($sub_src_id)) {
		$src_condition = " (b.source_id=".$src_id." and b.subsource_id=".$sub_src_id.")";
	}
...
<div class="contentMainDiv">
	<div class="contentDiv" style="width:98%;margin:auto;">	
		<table class="rptTbl">
            ...
            <tr> 
				<th colspan="12" class="main_heading"><?PHP echo $userProfile->getOff_level_name()." - ". $userProfile->getOff_loc_name() //Office Leval and Office Location Name?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?PHP echo $label_name[75]; //Office wise Report?></th>
			</tr>
			...
			<?php } ?>
			<tr> 
				<th colspan="12" class="search_desc"><b>As on <?php echo date("d-m-Y"); ?></b></th>
			</tr>
			...
			<tr>
				...
				<th rowspan="2" style="width:20%;"><?PHP echo 'Initiating Officer Name'; //Concerned Officer?> <br></th>
				<th style="width:10%;" colspan="4"><?PHP echo 'No Of Petitions'//'Pending';?></th>
			</tr>
			</tr>	
				<th style="width:10%;"><?PHP echo $label_name[37]//'Pending';?><br>(A)</th>
				<th style="width:10%;"><?PHP echo $label_name[11];//'Pending <=30 Days'?><br>(B)</th>
				<th style="width:10%;"><?PHP echo $label_name[12];//'Pending >30 Days <=60 Days'?><br>(C)</th>
				<th style="width:10%;"><?PHP echo $label_name[13];//'Pending >60 Days'?><br>(D)</th>
			</tr>
	...
	$dist_cond= "fn_single_district(".$userProfile->getDistrict_id().") md";
	$dist_cond_1= "fn_single_district(".$userProfile->getDistrict_id().") ";


	$dept=$userProfile->getDept_id(); // substr($sub_dept, 0,1);
	$subordinate_level = $userProfile->getOff_Level_id();
	...
	if ($subordinate_level==7) {
		...
		$dist_cond='(select a.state_id,a.state_id as off_location_id,a.state_name,a.state_name as off_loc_name,
		a.state_tname,a.state_tname as off_loc_tname,c.dept_id,c.dept_user_id,c.dept_desig_id,c.dept_desig_name,
		c.dept_desig_tname 
		from mst_p_state a 
		inner join vw_usr_dept_users_v c on c.off_loc_id=a.state_id and c.dept_id='.$dept.' and c.dept_desig_role_id=2
		and c.off_level_dept_id='.$userProfile->getOff_level_dept_id().' and c.pet_disposal ) aa ';
		...
		$dist_cond_1= "(select * from mst_p_sp_zone where zone_id=".$userProfile->getOff_Loc_id().")";
		$dist_cond='(select a.zone_id,a.zone_id as off_location_id,a.zone_name,a.zone_name as off_loc_name,a.zone_tname,a.zone_tname as off_loc_tname,c.dept_id,c.dept_user_id,c.dept_desig_id,c.dept_desig_name,c.dept_desig_tname from '.$dist_cond_1.' a
		inner join vw_usr_dept_users_v c on c.off_loc_id=a.zone_id and c.off_level_dept_id='.$userProfile->getOff_level_dept_id().' and c.pet_disposal
		and case 
		...
		$tbl_pet_list = "fn_pet_action_first_last_off_level(".$dept.",".$off_level_id.")";
		$dist_cond_1= "(select * from mst_p_sp_range where range_id=".$userProfile->getOff_Loc_id().")";
		 $dist_cond='(select a.range_id,a.range_name as off_loc_name,a.range_tname as off_loc_tname,c.dept_id,c.dept_user_id,c.dept_desig_id, c.dept_desig_name,
		c.dept_desig_tname  from '.$dist_cond_1.' a 
		inner join vw_usr_dept_users_v c on c.off_loc_id=a.range_id and c.off_level_dept_id='.$userProfile->getOff_level_dept_id().' and c.pet_disposal) aa';
		...
		inner join mst_p_taluk b on b.district_id=a.district_id and taluk_id='.$userProfile->getOff_Loc_id().') aa';		
		...
		c.dept_desig_tname from '.$dist_cond_1.' a 
		inner join mst_p_taluk b on b.district_id=a.district_id and taluk_id='.$userProfile->getOff_Loc_id().'
		inner join vw_usr_dept_users_v c on c.off_loc_id=b.taluk_id and c.off_level_dept_id='.$userProfile->getOff_level_dept_id().' and c.pet_disposal) aa';
		...
		$dist_cond='(select a.district_id,a.district_name,a.district_tname,b.block_id,b.block_name as off_loc_name,b.block_tname as off_loc_tname,c.dept_id,c.dept_user_id,c.dept_desig_id, c.dept_desig_name,
		c.dept_desig_tname from '.$dist_cond_1.' a 
		inner join mst_p_lb_block b on b.district_id=a.district_id and block_id='.$userProfile->getOff_Loc_id().'
		inner join vw_usr_dept_users_v c on c.off_loc_id=b.block_id and c.off_level_dept_id='.$userProfile->getOff_level_dept_id().' and c.pet_disposal) aa';
		
		$dist_params = 'aa.district_id,aa.district_name,aa.district_tname,';
		$off_level_params = 'aa.block_id as off_location_id,aa.off_loc_name,
		aa.off_loc_tname, aa.off_loc_tname,aa.dept_user_id,aa.dept_desig_id,aa.dept_desig_name,aa.dept_desig_tname,
		aa.dept_id,';
		$off_level_pk_select = 'block_id,action_entby';
		...
		$sql="WITH off_pet AS (
		select a.petition_id, a.action_type_code, a.off_loc_id as ".$off_level_pk.", b.petition_date,a.action_entby, a.l_action_type_code 
		from ".$tbl_pet_list." a
...
if($row_cnt!=0)
{
	...
	{
		if($_SESSION['lang']=='E'){  //c.dept_desig_name off_loc_name
			$desig_name_location=$row['dept_desig_name'].' - '.$row['off_loc_name'];
		}else{
			$desig_name_location=$row['dept_desig_tname'].' - '.$row['off_loc_tname'];
		}
		$pnd=$row['pnd']; 
	...
	<td class="desc"><?PHP echo $desig_name_location; ?></td>				
