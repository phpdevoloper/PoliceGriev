...
<?php
	...
	if ($office_type == "P") {
		...
		$particular_office_cond = "inner join vw_usr_dept_users_v a2 on a2.dept_user_id=a.pet_entby 
								   and a2.dept_id= ".$dept." 
								   and a2.off_level_id=".$off_level_id."
								   and a2.off_loc_id=".$off_loc_id."";
	}
...
<div class="contentMainDiv" style="width:98%;margin:auto;">
	<div class="contentDiv">	
		<table class="rptTbl">
            ...
            <tr> 
				<th colspan="8" class="main_heading"><?PHP echo $label_name[33].' - Action Taken by me and Accepted ' //Forwarded by me and pending with others'; //Department wise Report?></th>
			</tr>
            
            <?php if ($reporttypename != "") {?>
            <tr> 
				<th colspan="8" class="main_heading"><?PHP echo $reporttypename; //Report type name?></th>
			</tr>
            <?php } ?>
			...
			<tr>
				...
				<th><?PHP echo 'Petition No., Date and<br>Grievance Type'; //Petition No. & Date?></th>
				<th><?PHP echo $label_name[23]; //Petitioner's communication address?></th>
				<th><?PHP echo $label_name[24].' and '. $label_name[25]; //Source & Sub Source?></th>
				<th><?PHP echo $label_name[26]; //Grievance?></th>
				<th><?PHP echo 'Petition Department and<br>Office Address'; //Department & Address?></th>
	...
	$dist_cond=$userProfile->getOff_level_id()==1 ? "mst_p_district" : "fn_single_district(".$userProfile->getDistrict_id().")"; // fn_single_district
		...
		$sql="select petition_no, pet_action_id,petition_id, petition_date, source_name,subsource_name, subsource_remarks, 
		...
		action_type_code,to_whom,pet_type_name,dept_name
		from fn_petition_details(array(
		...
		".$particular_office_cond."
		where a1.action_entby = ".$userProfile->getDept_user_id()." --and a1.action_type_code in ('C','E','I','S') 
		".$cond3.$src_condition.$grev_condition.$grev_dept_condition.$petition_type_condition.$pet_community_condition.$special_category_condition." and  b.l_action_type_code='A')) order by griev_type_id, griev_subtype_id";
		//echo $sql;
	    $result = $db->query($sql);
...
if($row_cnt!=0)
	{
		...
		foreach($rowarray as $row)
		{
			...
			<?PHP  echo $row['petition_no']."<br>Dt.&nbsp;".$row['petition_date']; ?></a><br><?echo "<br>".$row['pet_type_name']."<br><br>".
			$row['griev_type_name'].":<br>".$row['griev_subtype_name'] ?></td>
			...
			<td class="desc" style="width:12%;"> <?PHP echo "Department: ".$row['dept_name']."&nbsp;"."<br><br>Address: ".$row['gri_address']; ?></td>
			...
			?>
			<tr><th colspan="8" style="text-align:right;font-size:15px;"><i><b>Report generated by:</b></i> <?PHP echo  $report_preparing_officer.' on '. date("d-m-Y h:i:sa");?></th></tr>
			<tr>
...
$i=1;
	...
	$all_office_cond = " and a.dept_id= ".$off_dept_id." ";
	//}
