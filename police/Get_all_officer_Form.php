...
function p1_returnDesignationSearchForEntry(petition_id, userID, offLoc_designName){
	$('#off_id').val(userID);
	$('#off_loc_id').val(userID);
	$('#off_name').val(offLoc_designName);
}
*/
function submitToDesign(){
	if($('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').val()>0){
		...
		if(open_form=="P1"){
			opener.p1_returnDesignationSearchForEntry( off_level_id, $('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').val(), off_loc_name);
		}
		else{
