...
if($check=='yes')
{
$pagetitle="Disposed Petitions Report";
?>
...
{ 
	...
	document.rpt_abstract.action="rptdist_disposed_petitions.php";
...
<?php
	...
	if(stripQuotes(killChars($_POST["p_from_date"]))!="")
		$p_from_date=stripQuotes(killChars($_POST["p_from_date"]));
	else  
		$p_from_date=stripQuotes(killChars($_SESSION["p_from_date"]));				

	if(stripQuotes(killChars($_POST["p_to_date"]))!="")
		$p_to_date=stripQuotes(killChars($_POST["p_to_date"]));
	else  
		$p_to_date=stripQuotes(killChars($_SESSION["p_to_date"]));
...
<div class="contentMainDiv" style="width:70%;margin:auto;">
	<div class="contentDiv">	
		<table class="rptTbl">
            ...
            <tr> 
				<th colspan="14" class="main_heading"><?PHP echo $userProfile->getDept_desig_name()." - ". $userProfile->getOff_loc_name() //Department wise Report?></th>
			</tr>
            <tr> 
			...
			<tr> 
				<th colspan="14" class="search_desc"><?php echo $label_name[48]; ?> &nbsp;&nbsp;
				<?php echo $from_date; ?>&nbsp;&nbsp;-&nbsp;&nbsp; <?php echo $to_date; ?>&nbsp;&nbsp;&nbsp;&nbsp;
				<?php echo $label_name[91]; ?> &nbsp;&nbsp;<?php echo $p_from_date; ?>&nbsp;&nbsp;-&nbsp;&nbsp;<?php echo $p_to_date; ?>
				</th>
			</tr>
...
<?php 
	...
	$pfromdate=explode('/',$p_from_date);
	$pday=$pfromdate[0];
	$pmnth=$pfromdate[1];
	$pyr=$pfromdate[2];
	$p_frm_dt=$pyr.'-'.$pmnth.'-'.$pday; 
			

	$ptodate=explode('/',$p_to_date);
	$pday=$ptodate[0];
	$pmnth=$ptodate[1];
	$pyr=$ptodate[2];
	$p_to_dt=$pyr.'-'.$pmnth.'-'.$pday; 
	...
	if ($delagated == "true") {
		...
		where l_action_entdt::date >= '".$p_frm_dt."'::date and  l_action_entdt::date <= '".$p_to_dt."'::date
		and petition_date::date >= '".$frm_dt."'::date and  petition_date::date <= '".$to_dt."'::date	
...
<input type="hidden" name="p_from_date" id="p_from_date" value="<?php echo $p_from_date; ?>" />
<input type="hidden" name="p_to_date" id="p_to_date" value="<?php echo $p_to_date; ?>" />
...
<div class="contentMainDiv" style="width:98%;margin:auto;">
	<div class="contentDiv">	
		<table class="rptTbl">
			...
			<tr> 
				<th colspan="8" class="search_desc"><?php echo $label_name[48]; ?> &nbsp;&nbsp;
				<?php echo $from_date; ?>&nbsp;&nbsp;-&nbsp;&nbsp; <?php echo $to_date; ?>&nbsp;&nbsp;&nbsp;&nbsp;
				<?php echo $label_name[91]; ?> &nbsp;&nbsp;<?php echo $p_from_date; ?>&nbsp;&nbsp;-&nbsp;&nbsp;<?php echo $p_to_date; ?>
				</th>
			</tr>
...
$i=1;
	...
	if ($delagated == "true") {
		...
		and petition_date::date >= '".$frm_dt."'::date and  petition_date::date <= '".$to_dt."'::date
		and l_action_type_code in ('A','R')	and d_action_entby=".$userProfile->getDept_user_id()." and d_action_type_code='D'		
		".$src_condition.$grev_condition.$grev_dept_condition.$petition_type_condition.$pet_community_condition.$special_category_condition.$instructions." 
