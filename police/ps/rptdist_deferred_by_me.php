...
function detail_view(frm_date,to_date,dept,dept_name,status,src_id,sub_src_id,gtypeid,gsubtypeid,grie_dept_id,off_cond_para,off_loc_name,off_src_id)
{ 
	...
	document.rpt_abstract.action="rptdist_deferred_by_me.php";
...
<div class="contentMainDiv" style="width:98%;margin:auto;">
	<div class="contentDiv">	
		<table class="rptTbl">
            ...
            <tr> 
				<th colspan="12" class="main_heading"><?PHP echo $label_name[33].' - '.'Deferred By Me' //Pending with myself at present '; //Department wise Report?></th>
			</tr>
			...
			<tr>
                ...
                <th rowspan="2"><?PHP echo 'Deferred and Pending'; //Closing Balance?></th>	
				<th colspan="3"><?PHP echo 'Deferred and Pending (in No. of Days)'; //Closing Balance?></th>	

			</tr>
			<tr>
			<th><?PHP echo $label_name[9]; //Accepted?></th>
			<th><?PHP echo $label_name[10]; //Rejected?></th>
			<th> <?PHP echo '<=30'; //Pending for more than 2 months?></th>
			<th> <?PHP echo '>30 and <=60'; //Pending for 2 months?></th>
			<th> <?PHP echo '> 60'; //Pending for 1 month?></th>
			  </tr>
	...
	$dist_cond=$userProfile->getOff_level_id()==1 ? "mst_p_district" : "fn_single_district(".$userProfile->getDistrict_id().")"; // fn_single_district
		...
		where (a1.l_action_entby=".$userProfile->getDept_user_id()." and a1.l_action_type_code='T') and ".$cond3.$src_condition.$grev_condition.$grev_dept_condition.$petition_type_condition.$pet_community_condition.$special_category_condition." ) 

		-- inner join pet_action_first_last oq on oq.petition_id=op.petition_id where op.rnk=1) 

		select * 

		from ( select aa.dept_desig_id, aa.dept_desig_name, bb.source_id, bb.source_name , bb.source_id as office_source_id,
		bb.source_tname, COALESCE(rwp.recd_cnt,0) as recd_cnt, COALESCE(cpa.cl_pet_a_cnt,0) as cl_pet_a_cnt, COALESCE(cpr.cl_pet_r_cnt,0) as cl_pet_r_cnt, COALESCE(pcbo.cl_pend_o_cnt,0) as cl_pend_o_cnt, COALESCE(pcb.cl_pend_cnt,0) as cl_pend_cnt, COALESCE(pcb.cl_pend_leq30d_cnt,0) as cl_pend_leq30d_cnt, COALESCE(pcb.cl_pend_gt30leq60d_cnt,0) as cl_pend_gt30leq60d_cnt, COALESCE(pcb.cl_pend_gt60d_cnt,0) as cl_pend_gt60d_cnt 
		...
		(select a1.source_id,count(*) as recd_cnt
		from off_pet a1
		group by a1.source_id) rwp on rwp.source_id=bb.source_id 

		left join -- closed petitions: status with 'A' 
		(select a1.source_id,count(*) as cl_pet_a_cnt
		from off_pet a1
		where a1.l_action_type_code='A'
		group by a1.source_id) cpa on cpa.source_id=bb.source_id 

		left join -- closed petitions: status with 'R' 
		(select a1.source_id,count(*) as cl_pet_r_cnt
		from off_pet a1
		where a1.l_action_type_code='R'
		group by a1.source_id) cpr on cpr.source_id=bb.source_id 

		left join -- pending with others 
		(select a1.source_id,count(*) as cl_pend_o_cnt
		from off_pet a1
		where a1.l_action_type_code not in ('A','R') and a1.l_to_whom <> ".$userProfile->getDept_user_id()."
		group by a1.source_id) pcbo on pcbo.source_id=bb.source_id 
		...
		from off_pet a1
		where a1.l_action_type_code not in ('A','R') and (a1.l_to_whom =  ".$userProfile->getDept_user_id()." or  (a1.l_action_entby=".$userProfile->getDept_user_id()." and a1.l_action_type_code='T'))
		group by a1.source_id) pcb on pcb.source_id=bb.source_id ) b_rpt 
	...
	<div class="contentDiv">	
		<table class="rptTbl">
			<thead>
				...
				<tr> 
				<th colspan="8" class="main_heading"><?PHP echo $label_name[33].' - '.'Deferred By Me' ;?> <?php echo " - ".$cnt_type; ?></th>
                </tr>
                <?php if($reporttypename!="") { ?>
                <tr>
