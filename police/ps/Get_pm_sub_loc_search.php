...
<title>Subject & Location based Search</title>
<style>
.stop-scrolling {
  height: 100%;
  overflow: hidden;
...
<table class="existRecTbl" style="border-top-style: solid;">
	<thead>
    <tr>
    	<th style="background-color: #BC7676; color: #FFFFFF; font-size: 150%;" colspan="2">Subject & Location based search</th>
    </tr>
	</thead>
</table>
...
function randomHex() {
	...
	if(!empty($area)){
		...
		$codn.=" and  griev_type_id= $main";
	}if(!empty($sub)){
		$codn.=" and  griev_subtype_id= $sub";
	}
	if(!empty($street)){
...
<input type="button" class="button" value="Exit" id="p1_exit" name="p1_exit">
 ...
 <input type="hidden" name="mobile_number1" id="mobile_number1" value="<?php echo $mobile_number;?>"/>
 <?php
}else{
	?>
