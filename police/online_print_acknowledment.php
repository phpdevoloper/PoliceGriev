...
$sql = "select petition_id from pet_master where petition_no='".$petno."'";
	$result=$db->query($sql);
	...
	$url='localhost/pol/getPetitionStatusQR.php?pet_id='.$row['petition_id'];
...
$(document).ready(function(){
	...
	$.ajax({
		...
		if (e_mail != '') {
			var comm_label = "<?php echo $lang['MOBILE_NUMBER_LABEL'].' & '. $lang['Email_Label']; ?>";
			var comm_detail = mobile_number+' & '+e_mail;
		} else {
		...
		var  doc_tr = show_doc_show+
					  ...
					  "<td >"+"<span id='span_dwnd'  onClick='download_document("+$(xml).find('Document_id').eq(0).text()+")'>"+$(xml).find('Document_name').eq(0).text()+"</span></td>"+
						"</tr>";
...
"<tr id='he_gov' style='display:none;'>"+
	"<td colspan='2' class='heading text-center' style='font-weight: 700;font-size: 20px;'>"+" <label>Tamil Nadu Police- Senior Police Officers Petition System(SPOPS) - Online Status </label>"+
	"</td>"+
"</tr>"+
"<tr>"+
	"<td colspan='2' class='heading text-center' style='font-weight: 700;font-size: 20px;'>"+"<?php echo $lang['ACKNOWLEDGEMENT_LABEL']; ?>"+"</td><img height='70' width='70' src='theme/images/emblem-dark.png' id='prnt_img' align='left'/>"+"<img src='"+"<?php echo $PNG_WEB_DIR.basename($filename); ?>"+"' id='prnt_img' align='right' height='70' width='70' style='margin-right:30px' />"+
"</tr>"+
...
"<tr>"+
	...
	"<td><b>"+$(xml).find('Petition_No').eq(0).text()+'</b>'+$(xml).find('Date').eq(0).text()+"</td>"+
"</tr>"+
"<tr>"+
	"<td >"+"<?php echo $lang['SOURCE_SUB_SOURCE_AND_REMARKS_LABEL1']; ?>"+"</td>"+
...
"</tr>"+
"<tr>"+
	"<td>"+"<?php echo 'Submission level'; ?>"+"</td>"+
	"<td>"+$(xml).find('Petition_With').eq(0).text()+"</td>"+
"</tr>"+
"<tr>"+
...
function goback(){
	window.location = 'online_acknowledment.php';	
}
function download_document(url){
source='P';
	//window.location.href="https://locahost/police/pm_petition_doc_download.php?doc_id="+url+"&source="+source;
	//window.location.href="http://localhost/polie/pm_petition_doc_download.php?doc_id="+url+"&source="+source;
	window.location.href="http://14.139.183.34/polie/pm_petition_doc_download.php?doc_id="+url+"&source="+source;
}
...
<?php echo '<b>'.$petition_no.' </b>' .$lang['ACKNOWLEDGEMENT_CHECK_STATUS1_LABEL'];?>
		...
		<tr>
			<td  class="text-center pad_t" >
			 <a class="btn btn-primary fa fa-sign-out"  name="back" id="back" onClick="goback();">&nbsp;<?PHP echo $lang['Back_Button_Label']; //View ?></a>
			</td>
		</tr>
		<tr>
	...
	<thead>
		<tr id='btn_row' >
			<td colspan="7" class="text-center pad_t" >
			 <a class="btn btn-primary"  name="print" id="print" onClick="return print_page();"><i class="glyphicon glyphicon-print"></i>&nbsp;<?PHP echo $lang['Print_Button_Label']; //View ?></a> 
			 <a class="btn btn-primary"  name="back" id="back" onClick="goback();"><i class="glyphicon glyphicon-log-out"></i>&nbsp;&nbsp;<?PHP echo $lang['Back_Button_Label']; //View ?></a>
			</td>
		</tr>
