...
echo $page->generateXMLTag('Email_Label', $row['comm_email']);
	$link_pet_status="select petition_no, org_petition_no, to_char(l_action_entdt, 'dd-mm-yyyy hh12:mi:ss PM')::character varying AS action_date, l_action_type_code, action_type_name, action_type_tname, l_action_type_code,dept_desig_name,dept_desig_tname,location_name,location_tname, l_to_whom,to_dept_desig_name,to_dept_desig_tname,to_location_name,to_location_tname
		from fn_clubbed_petition_status((select petition_id from pet_master where petition_no='$petno'))";
		$status_rs=$db->query($link_pet_status);
		$status_rowarray = $status_rs->fetchall(PDO::FETCH_ASSOC);
		$k=1;
$link_petition_status_msg='';
		foreach($status_rowarray as $status_row){
			...
			if($status_row['l_action_type_code']=='A'||$status_row['l_action_type_code']=='R'){
				...
				$link_petition_status_msg.= $k++.") ".$status_row['petition_no']." Status: <lim style='color:".$color."'>".$status_row['action_type_name']."</lim> on ".$status_row['action_date']." by ".$action_ent." &emsp;<br>   ";
			}else{
				//$link_petition_status.= $k++.") ".$status_row[petition_no]." Status:  Under Process &emsp;<br>";
				if($status_row['action_type_name']!=''){
				$link_petition_status_msg.= $k++.") ".$status_row['petition_no']." Status: ".$status_row['action_type_name']." on ".$status_row['action_date']." by ".$action_ent." <b>".$to_action_ent."</b>&emsp;<br>";
				}else{
					$link_petition_status_msg.= $k++.") ".$status_row['petition_no']." Status: No Action Taken So far.&emsp;<br>";
				}
			}
		}
			echo $page->generateXMLTag('link_petition_status', $link_petition_status_msg);
...
echo $page->generateXMLTag('action_doc_name', $key['action_doc_name']);
	...
	if ($stype != 's') {
	$query="select * from pet_master_ext_link where petition_id=(select petition_id from pet_master where petition_no='$petno')";
	$result = $db->query($query);
		$rowarray = $result->fetchall(PDO::FETCH_ASSOC);
		if (sizeof($rowarray)>0)
		{
			foreach($rowarray as $row)
			{
				...
				if($pet_ext_link_id==1){
					$pet_ext_link_name="FIR";
				}else if($pet_ext_link_id==2){
					$pet_ext_link_name="CSR";
				}
				echo $page->generateXMLTag('pet_ext_link_name', $pet_ext_link_name);
				echo $page->generateXMLTag('fir_csr_year',$row['fir_csr_year'] );
				echo $page->generateXMLTag('pet_ext_link_no',$row['pet_ext_link_no'] );
				$district_id=$row['district_id'];
				$dis_query="select district_name from mst_p_district where district_id=$district_id";
				$dis_result = $db->query($dis_query);
				$dis_arr = $dis_result->fetchall(PDO::FETCH_ASSOC);
				foreach($dis_arr as $dis)
				{
				echo $page->generateXMLTag('fir_district_name', $dis['district_name']);
				}
				$circle_id=$row['circle_id'];
				$cir_query="select circle_name from mst_p_sp_circle where circle_id=$district_id";
				$cir_result = $db->query($cir_query);
				$cir_arr = $cir_result->fetchall(PDO::FETCH_ASSOC);
				foreach($cir_arr as $cir)
				{
				echo $page->generateXMLTag('fir_circle_name', $cir['circle_name']);
				}
			}
		...
		echo $page->generateXMLTag('Pending_Period', $pend_period);		//echo $query;exit;
