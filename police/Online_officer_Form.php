...
{	$('#alpha_row').hide();
	...
	$("#p1_submit").click(function(){
		...
		if(open_form=="P1"){
			opener.p1_returnDesignationSearch( off_level_id, $('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').val(), off_loc_name);
			//window.p1_returnDesignationSearch( off_level_id, $('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').val(), off_loc_name);
		}
		else{
...
function p1_returnDesignationSearch(petition_id, userID, offLoc_designName){
	$('#off_d_id').val(userID);alert(offLoc_designName);
	$('#email').val(offLoc_designName);
}
function p1_loadGrid(pageNo, pageSize){
	...
	$.ajax({
		type: "POST",
		dataType: "xml",
		url: "Online_all_officer_Form_Action.php",  
		data: param,  
...
function submitToDesign(){alert("submit1");
	if($('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').val()>0){
		var off_level_id=$("#off_level_id").val();
		var open_form='<?PHP echo $_REQUEST['open_form']?>';
		var off_loc_name = $('#tr_'+$('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').val()).find("td").eq(1).html() +" / "+
			$('#tr_'+$('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').val()).find("td").eq(1).html();
		if(open_form=="P1"){alert("submit");
			opener.p1_returnDesignationSearch( off_level_id, $('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').val(), off_loc_name);
		}
		else{
			opener.p3_returnDesignationSearch( off_level_id, $('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').val(), off_loc_name);
 ...
 /*function submitToDesign(){
	if($('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').val()>0){
		...
		var off_loc_name = $('#tr_'+$('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').val()).find("td").eq(1).html() +" / "+
			$('#tr_'+$('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').val()).find("td").eq(1).html();
		if(open_form=="P1"){
			window.parent.document.getElementById('off_level_id').value=$('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').value;alert();
		}
		else{
			opener.p3_returnDesignationSearch( off_level_id, $('input[name=off_loc_id]:checked', '#p1_off_loc_design_search').val(), off_loc_name);
		}
...
<input type="hidden" name="pre" id="pre" value="<?PHP echo $_REQUEST['pre']?>"/>
...
<table class="existRecTbl" style="border-top-style: solid;">
	...
	<?php
		 ...
		 $sql="select off_hier[13] as dist_id from vw_usr_dept_users_v_sup where dept_off_level_pattern_id='".$_REQUEST['off_level_id']."' and dept_desig_id='".$_REQUEST['off_level_dept_id']."'";
		...
		if($_REQUEST['off_level_dept_id']!=3){
		$sql = "SELECT * FROM mst_p_district where district_id!=2 order by case when district_id<0 then 1 else 0 end,district_name";
		...
		}else if($_REQUEST['off_level_dept_id']==3){
			print("<option value='2'>Chennai</option>");
		}
	?>
    </select>
	</span>
	...
	<tr>
	<td style="text-align:center;background-color:#FFFFFF;" colspan="2" id="alpha_row">
